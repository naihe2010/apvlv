...
namespace apvlv
{
    ...
    ApvlvDoc::blank (ApvlvImage *image)
    {
      ...
      ApvlvDocCache *cache = mCurrentCache[image->mId];
            ...
            if (mDisplayType == DISPLAY_TYPE_IMAGE)
              {
                scrollto (0.5);
                auto x = 0;
                auto y = double (mCurrentCache[mCurrentImage->mId]->getheight ()) / 2;
    ...
    void ApvlvDoc::updateLastPoint (gdouble x, gdouble y)
    {
      debug("update last point: %.0f-%.0f\n", x, y);
      mLastPoint = {x, y};
    }

    void ApvlvDoc::updateCurPoint (gdouble x, gdouble y, gboolean updateLast)
    {
 ...
 * */
    ...
    void
    ApvlvDocCache::preparelines (gint x1, gint y1, gint x2, gint y2)
    {
