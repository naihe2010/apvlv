...
namespace apvlv
{
  ...
  ApvlvView::ApvlvView (const char *filename) : mHasTabs(false), mCurrTabPos (-1)
  {
    mProCmd = 0;
  ...
  void
    ApvlvView::newtab (const char *filename)
      {
  ...
  int
    ApvlvView::new_tabcontext (ApvlvCore *core, bool insertAfterCurr)
      {
  ...
  ApvlvCore *
    ApvlvView::hasloaded (const char *abpath, int type)
      {
        GSList *node;
        ApvlvCore *core;
  ...
  gint
    ApvlvView::apvlv_view_keypress_cb (GtkWidget * wid, GdkEvent * ev, ApvlvView *view)
      {
  ...
  gint
    ApvlvView::apvlv_view_commandbar_cb (GtkWidget * wid, GdkEvent * ev, ApvlvView *view)
      {
    ...
    ApvlvView::updatetabname ()
      {
        ...
        if (mTabList[mCurrTabPos].numwindows > 1)
          g_snprintf (tagname, sizeof tagname, "[%d] %s", mTabList[mCurrTabPos].numwindows, gfilename);
        else
